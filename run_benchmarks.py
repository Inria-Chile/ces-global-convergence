...
def run_benchmarks_production(R=30, dimensions=[1, 2, 10, 30], case="uniform", threshold=1e-2):
    ...
    for d in dimensions:
        ...
        for i in range(R):
            ...
            # --- 3. CBO (Consensus Based Optimization) ---
            ...
            # --- 4. SGD (SOLO SI d <= 2) ---
            if d <= 2:
                x0_sgd = np.mean(x0, axis=0) # Punto de partida: centro de la nube
    ...
    # Guardar en Parquet (Binario puro, sin pérdida de precisión)
    df = pd.DataFrame(results)
    output_name = f"data/benchmarks_final_{case}.parquet"
    df.to_parquet(output_name, engine='pyarrow')
    print(f"\n¡Listo! Datos guardados en {output_name}")
def to_latex_scientific_compact(val, std):
    """
    Formato Pro: 1.2 \times 10^{-3} (\pm 4.0 \times 10^{-4})
    """
    ...
    def fmt_latex(x):
        if x == 0: 
            return "0"
        if 0.01 <= abs(x) < 100:
            # Para números en rangos "normales", evitamos la potencia de 10
            return f"{x:.2f}"
            
        exponent = int(np.floor(np.log10(abs(x))))
        coeff = x / (10**exponent)
        
        # Retorna el formato \times 10^{exp}
...
def generate_full_paper_table(df_uni, df_shi, filename="tables/final_results.tex"):
    # 1. Preparación de datos (Igual a la lógica anterior)
    ...
    # --- CONSTRUCCIÓN DEL STRING LATEX ---
    latex = []
    # Encabezado idéntico a tu ejemplo
    ...
    for scenario in ['Uniform', 'Shifted']:
        ...
        for d in dims:
            ...
            # --- LÓGICA DE NEGRITAS (BOLD) ---
            # Buscamos cuál de los métodos tiene el error medio más bajo en esta fila
            ...
            for m in methods:
                try:
                    ...
                    if pd.isna(m_mean):
                        ...
                        # Si es el mejor, envolvemos en \textbf
                        if m == best_method:
...
if __name__ == "__main__":
    #run_benchmarks_production(R=30, dimensions=[1, 2, 10, 30], case="uniform", threshold=1e-2)
    #run_benchmarks_production(R=30, dimensions=[1, 2, 10, 30], case="shifted", threshold=1e-2)
